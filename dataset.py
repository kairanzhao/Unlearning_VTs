...
def svhn_dataloaders_idx(
    ...
    return train_loader, val_loader, train_idx

class SVHNWithID(datasets.SVHN):
    def __init__(self, *args, **kwargs):
        super(SVHNWithID, self).__init__(*args, **kwargs)
    ...
    def __getitem__(self, index):
        ...
        img, label = super(SVHNWithID, self).__getitem__(index)
...
def svhnwithids_dataloaders(
    ...
    train_set = SVHNWithID(data_dir, split="train", transform=train_transform, download=True)

    test_set = SVHNWithID(data_dir, split="test", transform=test_transform, download=True)
    ...
    train_set.data = train_set_copy.data[train_idx]
    train_set.labels = train_set_copy.labels[train_idx]
    train_set.ids = [train_set_copy.ids[i] for i in train_idx]
...
class TinyImageNet:
    ...
    def __init__(self, args, normalize=False):
        ...
        if args.no_aug:
            ...
            print("Data augmentation = randomcrop(64,4) + randomhorizontalflip")
    ...
    def organize_data(self):
        test_original_path = os.path.join(self.args.data_dir, "test_original")
        if not os.path.exists(test_original_path):
            if os.path.exists(self.test_path):
                os.rename(self.test_path, test_original_path)
            ...
            paths = glob.glob(os.path.join(self.args.data_dir, "val/images/*"))
            for path in paths:
                file = path.split("/")[-1]
                folder = val_dict[file]
                if not os.path.exists(os.path.join(self.val_path, folder)):
                    os.mkdir(os.path.join(self.val_path, folder))
                    os.mkdir(os.path.join(self.val_path, folder, "images"))
                if not os.path.exists(os.path.join(self.test_path, folder)):
                    os.mkdir(os.path.join(self.test_path, folder))
                    os.mkdir(os.path.join(self.test_path, folder, "images"))

            for path in paths:
                file = path.split("/")[-1]
                folder = val_dict[file]
                if len(glob.glob(os.path.join(self.val_path, folder, "images", "*"))) < 25:
                    dest = os.path.join(self.val_path, folder, "images", file)
                else:
                    dest = os.path.join(self.test_path, folder, "images", file)
                move(path, dest)
...
class TinyImageNetDatasetWithID(Dataset):
    def __init__(self, image_folder_set, norm_trans=None, start=0, end=-1):
        self.imgs = []
        self.targets = []
        self.transform = image_folder_set.transform
        ...
        self.imgs = torch.stack(self.imgs)
        # generate a unique id for each image based on its index
        self.ids = np.arange(len(self.imgs))
    ...
    def __getitem__(self, idx):
        if self.transform is not None:
            return self.transform(self.imgs[idx]), self.targets[idx], self.ids[idx]
        else:
            return self.imgs[idx], self.targets[idx], self.ids[idx]

class TinyImageNetWithID:
    """
    TinyImageNet dataset.
    """
    ...
    def data_loaders(
        ...
        train_set = ImageFolder(self.train_path, transform=self.tr_train)
        train_set = TinyImageNetDatasetWithID(train_set, self.norm_layer)
        test_set = ImageFolder(self.test_path, transform=self.tr_test)
        test_set = TinyImageNetDatasetWithID(test_set, self.norm_layer)
        train_set.targets = np.array(train_set.targets)
        train_set.targets = np.array(train_set.targets)
        rng = np.random.RandomState(seed)
        valid_set = copy.deepcopy(train_set)
...
def cifar10_dataloaders_idx(
    ...
    if no_aug:
        ...
        if 'swin' in arch or 'vit' in arch:
            train_transform = transforms.Compose(
                [
                    transforms.RandomCrop(32, padding=4),
                    transforms.RandomHorizontalFlip(),
...
def cifar10withids_dataloaders(
    batch_size=128,
    data_dir="datasets/cifar10",
    num_workers=2,
    random_to_replace: int = None,
    ...
    data_dir = data_dir + '/cifar10'
    train_set = CIFAR10WithID(root=data_dir, train=True, download=False, transform=train_transform)
    test_set  = CIFAR10WithID(root=data_dir, train=False, download=False, transform=test_transform)
    ...
    valid_set.data = train_set_copy.data[valid_idx]
    valid_set.targets = train_set_copy.targets[valid_idx]
    valid_set.ids = [train_set_copy.ids[i] for i in valid_idx]
...
class CIFAR100WithID(datasets.CIFAR100):
    def __init__(self, *args, **kwargs):
        super(CIFAR100WithID, self).__init__(*args, **kwargs)
        # Generate a unique ID for each image based on its index
        self.ids = list(range(len(self.data)))

    def __getitem__(self, index):
        # Get the original CIFAR-10 data and label
        img, label = super(CIFAR100WithID, self).__getitem__(index)
...
def cifar100withids_dataloaders(
    batch_size=128,
    data_dir="datasets/cifar100",
    num_workers=2,
    random_to_replace: int = None,
    ...
    data_dir = data_dir + '/cifar100'
    train_set = CIFAR100WithID(root=data_dir, train=True, download=False, transform=train_transform)
    test_set  = CIFAR100WithID(root=data_dir, train=False, download=False, transform=test_transform)
